import React, { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Github, Linkedin, Mail, Instagram, ExternalLink } from "lucide-react";

const Sidebar = () => {
  const [scrollPosition, setScrollPosition] = useState(0);
  const [isFooterVisible, setIsFooterVisible] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      const scrollY = window.scrollY;
      const windowHeight = window.innerHeight;
      const documentHeight = document.documentElement.scrollHeight;

      // Calculate total scroll percentage
      const maxScrollableHeight = documentHeight - windowHeight;
      const scrollPercent = (scrollY / maxScrollableHeight) * 100;

      // Add hysteresis: trigger at 97%, but only hide when below 95%
      if (scrollPercent >= 97) {
        setIsFooterVisible(true);
      } else if (scrollPercent < 95) {
        setIsFooterVisible(false);
        // Calculate scroll progress for sidebar movement (0 to 1)
        // Icons move from 20% to 55% of viewport as we scroll from 0% to 95%
        const progress = Math.min(Math.max(scrollPercent / 95, 0), 1);
        setScrollPosition(progress);
      }
      // Between 95-97%, maintain current state (no flicker)
    };

    window.addEventListener("scroll", handleScroll);
    handleScroll();

    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  const socials = [
    {
      icon: <Github size={20} />,
      href: "https://github.com/Kishor0513",
      label: "GitHub",
    },
    {
      icon: <Linkedin size={20} />,
      href: "https://linkedin.com/in/kishor-chaudhary",
      label: "LinkedIn",
    },
    {
      icon: <Instagram size={20} />,
      href: "https://www.instagram.com/kishor0513/",
      label: "Instagram",
    },
    {
      icon: <ExternalLink size={20} />,
      href: "https://linktr.ee/kishor0513",
      label: "Linktree",
    },
    {
      icon: <Mail size={20} />,
      href: "mailto:kishorc2000@gmail.com",
      label: "Email",
    },
  ];

  return (
    <AnimatePresence mode="wait">
      {!isFooterVisible && (
        <motion.div
          key="sidebar"
          initial={{ x: 100, opacity: 0 }}
          animate={{
            x: 0,
            opacity: 1,
            transition: { duration: 0.5 },
          }}
          exit={{ x: 100, opacity: 0, transition: { duration: 0.3 } }}
          style={{
            top: `${20 + scrollPosition * 35}%`,
          }}
          className="fixed right-6 z-50 hidden xl:flex flex-col gap-6"
        >
          <div className="flex flex-col gap-6 items-center">
            {socials.map((social, index) => (
              <motion.a
                key={index}
                href={social.href}
                target="_blank"
                rel="noopener noreferrer"
                initial={{ opacity: 0, x: 50 }}
                animate={{
                  opacity: 1,
                  x: 0,
                  transition: { delay: index * 0.1 },
                }}
                whileHover={{ scale: 1.2, x: -5 }}
                className="text-gray-400 hover:text-primary transition-colors p-2 glass-panel rounded-xl border border-white/5 hover:border-primary/50 group relative liquid-glass"
              >
                {social.icon}
                <span className="absolute right-full mr-4 px-2 py-1 bg-primary text-dark text-xs font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                  {social.label}
                </span>
              </motion.a>
            ))}
          </div>
          <div className="w-px h-24 bg-gradient-to-b from-primary/50 to-transparent mx-auto mt-4" />
        </motion.div>
      )}

      {isFooterVisible && (
        <motion.div
          key="footer"
          className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50 hidden xl:block"
        >
          {" "}
          <div className="flex gap-6 items-center">
            {socials.map((social, index) => (
              <motion.a
                key={index}
                href={social.href}
                target="_blank"
                rel="noopener noreferrer"
                initial={{
                  x: window.innerWidth * 0.4,
                  y: -window.innerHeight * 0.3,
                  opacity: 0,
                  scale: 0.5,
                }}
                animate={{
                  x: 0,
                  y: 0,
                  opacity: 1,
                  scale: 1,
                  transition: {
                    delay: index * 0.15,
                    duration: 0.8,
                    type: "spring",
                    stiffness: 100,
                    damping: 15,
                  },
                }}
                exit={{
                  x: window.innerWidth * 0.4,
                  y: -window.innerHeight * 0.3,
                  opacity: 0,
                  scale: 0.5,
                  transition: {
                    delay: (4 - index) * 0.1,
                    duration: 0.6,
                  },
                }}
                whileHover={{ scale: 1.2, x: -5 }}
                className="text-gray-400 hover:text-primary transition-colors p-2 glass-panel rounded-xl border border-white/5 hover:border-primary/50 group relative liquid-glass"
              >
                {social.icon}
                <span className="absolute bottom-full mb-4 left-1/2 transform -translate-x-1/2 px-2 py-1 bg-primary text-dark text-xs font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                  {social.label}
                </span>
              </motion.a>
            ))}
          </div>
        </motion.div>
      )}
    </AnimatePresence>
  );
};

export default Sidebar;
